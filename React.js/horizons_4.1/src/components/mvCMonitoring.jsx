/**
 * mvCMonitoring.jsx
 * Dashboard monitoring Power Meter untuk ruangan MV-C (Dynamic DB Render & MVA Parity).
 */
...
import { getMVCData } from "@/services/apiService";
...
const MvCMonitoring = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const result = await getMVCData(params);
      if (result.success) {
        const statusMap = {};
      ...
      console.error("[mvCMonitoring] Failed to fetch data:", error);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    const connectWs = () => {
      ...
      catch (err) { console.error("[WS-MVC] WebSocket constructor error:", err); return; }
  ...
  const downloadExcel = useCallback(() => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "MV-C Data");
    XLSX.writeFile(wb, `MVC_${resolution}_${periodStr}.xlsx`);
  ...
  const getBarChartOptions = (title, unit, color, lastDataTs = null) => {
    ...
    return {
      chart: { id: `chart-mvc-${title}`, background: "transparent", foreColor: "#64748b",
  ...
  const getLineChartOptions = (title, unit, color, lastDataTs = null, discreteMarkers = []) => {
    const { dayStart, axisMax } = getAxisBounds(lastDataTs);
    return {
      chart: { id: `chart-mvc-${title}`, background: "transparent", foreColor: "#64748b", toolbar: { show: false }, zoom: { enabled: false }, animations: { enabled: true, speed: 400 } },
  ...
  const renderCombinedVoltageChart = () => {
    ...
    const options = {
      chart: { id: `chart-mvc-voltage-combined`, background: "transparent", foreColor: "#64748b", toolbar: { show: false }, zoom: { enabled: false }, animations: { enabled: true, speed: 400 } },
  ...
  const renderCombinedThdChart = () => {
    ...
    const options = {
      chart: { id: `chart-mvc-thd-combined`, background: "transparent", foreColor: "#64748b", toolbar: { show: false }, zoom: { enabled: false }, animations: { enabled: true, speed: 400 } },
      theme: { mode: "dark" }, title: { text: undefined },
      xaxis: { type: "datetime", min: dayStart, max: axisMax, labels: { show: false }, axisBorder: { show: false }, axisTicks: { show: false }, tooltip: { enabled: false } },
  ...
  const renderCombinedCurrentChart = () => {
    ...
    const options = {
      chart: { id: `chart-mvc-current-combined`, background: "transparent", foreColor: "#64748b", toolbar: { show: false }, zoom: { enabled: false }, animations: { enabled: true, speed: 400 } },
      theme: { mode: "dark" }, title: { text: undefined },
      xaxis: { type: "datetime", min: dayStart, max: axisMax, labels: { show: false }, axisBorder: { show: false }, axisTicks: { show: false }, tooltip: { enabled: false } },
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#0d1117] text-slate-200 overflow-hidden">
      <header className="bg-[#0d1117] border-b border-slate-700 px-5 py-4 flex justify-between items-center z-10 shrink-0">
        ...
        <h1 className="font-mono text-2xl font-bold tracking-widest text-blue-400 italic">DASHBOARD MV-C</h1>
