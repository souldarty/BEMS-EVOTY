/**
 * mvAMonitoring.jsx
 * Dashboard monitoring Power Meter untuk ruangan MV-A (Dynamic DB Render).
 */
...
const getThisMonthStr = () => new Date().toISOString().slice(0, 7); // YYYY-MM
const getThisYearStr = () => new Date().getFullYear().toString();   // YYYY
...
const MvAMonitoring = () => {
  ...
  // REVISI: Mengubah list device dari hardcode menjadi state dinamis
  ...
  const [wsLiveData,  setWsLiveData]  = useState({});
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const result = await getMVAData(params);
      if (result.success) {
        const statusMap = {};
        (result.devices || []).forEach((d) => {
          if (d.status !== null) statusMap[d.subgroup_mv] = d.status;
        });
        setMeterStatus(statusMap);
        setAllData(result.data || {});
        // REVISI: Assign daftar list device yang dikirim oleh backend
        setMeterData(result.devices || []); 
  ...
  // Auto-refresh interval (Only applies to Today's Daily Data)
  useEffect(() => {
  ...
  useEffect(() => {
    const connectWs = () => {
      ...
      catch (err) { console.error("[WS-MVA] WebSocket constructor error:", err); return; }
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === "realtime_update" && msg.devices) setWsLiveData(msg.devices);
        } catch (err) {}
      };
      ws.onclose = (event) => {
  ...
  const downloadExcel = useCallback(() => {
    ...
    Object.entries(allData).forEach(([device, logs]) => {
      logs.forEach((log) => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "MV-A Data");
    XLSX.writeFile(wb, `MVA_${resolution}_${periodStr}.xlsx`);
  }, [allData, resolution, selectedDate, selectedMonth, selectedYear]);
  ...
  const getAxisBounds = (lastDataTs) => {
    ...
    const axisMax = lastDataTs && lastDataTs > dayStart && resolution === 'daily' ? lastDataTs : dayEnd;
  ...
  const getBarChartOptions = (title, unit, color, lastDataTs = null) => {
    ...
    return {
      chart: { id: `chart-mva-${title}`, background: "transparent", foreColor: "#64748b",
  ...
  const getLineChartOptions = (title, unit, color, lastDataTs = null, discreteMarkers = []) => {
    const { dayStart, axisMax } = getAxisBounds(lastDataTs);
    return {
      chart: { id: `chart-mva-${title}`, background: "transparent", foreColor: "#64748b", toolbar: { show: false }, zoom: { enabled: false }, animations: { enabled: true, speed: 400 } },
  ...
  const handleCardClick = (device) => {
    if (selectedDevice?.subgroup_mv === device.subgroup_mv) { setSelectedDevice(null); return; }
    setSelectedDevice(device);
    setTimeout(() => graphRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 80);
  };

  // REVISI: Pembagian Card sekarang mengacu pada state dinamis `meterData`
  const cardRows = [];
  for (let i = 0; i < meterData.length; i += CARD_COLS) cardRows.push(meterData.slice(i, i + CARD_COLS));
  const selectedGlobalIdx = selectedDevice ? meterData.findIndex((d) => d.subgroup_mv === selectedDevice.subgroup_mv) : -1;
  const selectedRowIdx    = selectedGlobalIdx >= 0 ? Math.floor(selectedGlobalIdx / CARD_COLS) : -1;

  const renderChartPanel = (title, unit, color, data, chartType = "bar") => {
    const stats = getChartStats(data);
    const lastDataTs = data.reduce((max, [ts, val]) => (val > 0 ? Math.max(max, ts) : max), 0) || null;
    let options; let seriesData = data;

    if (chartType === "line") {
      const lastNonZeroIdx  = data.reduce((last, [, val], i) => (val > 0 ? i : last), -1);
      if (lastNonZeroIdx !== -1) seriesData = data.slice(0, lastNonZeroIdx + 1);
    ...
    return (
      <div className="bg-[#0d1117] border border-slate-700/60 rounded-xl p-4 flex flex-col">
        ...
        <div style={{ height: "200px" }}>
          <Chart options={options} series={[{ name: selectedDevice?.subgroup_mv || "", data: seriesData }]} type={chartType} height="100%" />
        </div>
      </div>
    );
  };
  ...
  const renderCombinedVoltageChart = () => {
    ...
    const lastDataTs = Math.max(
      dataL1.reduce((max, [ts, val]) => (val > 0 ? Math.max(max, ts) : max), 0),
      dataL2.reduce((max, [ts, val]) => (val > 0 ? Math.max(max, ts) : max), 0),
      dataL3.reduce((max, [ts, val]) => (val > 0 ? Math.max(max, ts) : max), 0)
    ) || null;

    const { dayStart, axisMax } = getAxisBounds(lastDataTs);
    const colors = ["#3b82f6", "#60a5fa", "#93c5fd"]; 
    const options = {
      chart: { id: `chart-mva-voltage-combined`, background: "transparent", foreColor: "#64748b", toolbar: { show: false }, zoom: { enabled: false }, animations: { enabled: true, speed: 400 } },
      ...
      grid: { borderColor: "#1e293b" }, dataLabels: { enabled: false },
    };

    return (
      <div className="bg-[#0d1117] border border-slate-700/60 rounded-xl p-4 flex flex-col col-span-2">
        ...
        <div style={{ height: "200px" }}>
          <Chart options={options} series={[{ name: "Voltage L1-L2", data: trimData(dataL1) }, { name: "Voltage L2-L3", data: trimData(dataL2) }, { name: "Voltage L3-L1", data: trimData(dataL3) }]} type="line" height="100%" />
        </div>
      </div>
    );
  };
  ...
  const renderCombinedThdChart = () => {
    ...
    const options = {
      chart: { id: `chart-mva-thd-combined`, background: "transparent", foreColor: "#64748b", toolbar: { show: false }, zoom: { enabled: false }, animations: { enabled: true, speed: 400 } },
      theme: { mode: "dark" }, title: { text: undefined },
      xaxis: { type: "datetime", min: dayStart, max: axisMax, labels: { show: false }, axisBorder: { show: false }, axisTicks: { show: false }, tooltip: { enabled: false } },
    ...
    return (
      <div className="bg-[#0d1117] border border-slate-700/60 rounded-xl p-4 flex flex-col col-span-2">
        ...
        <div style={{ height: "200px" }}>
          <Chart options={options} series={[{ name: "THD L1", data: trimData(dataL1) }, { name: "THD L2", data: trimData(dataL2) }, { name: "THD L3", data: trimData(dataL3) }]} type="line" height="100%" />
        </div>
      </div>
    );
  };
  ...
  const renderCombinedCurrentChart = () => {
    ...
    const options = {
      chart: { id: `chart-mva-current-combined`, background: "transparent", foreColor: "#64748b", toolbar: { show: false }, zoom: { enabled: false }, animations: { enabled: true, speed: 400 } },
      theme: { mode: "dark" }, title: { text: undefined },
      xaxis: { type: "datetime", min: dayStart, max: axisMax, labels: { show: false }, axisBorder: { show: false }, axisTicks: { show: false }, tooltip: { enabled: false } },
    ...
    return (
      <div className="bg-[#0d1117] border border-slate-700/60 rounded-xl p-4 flex flex-col col-span-2">
        ...
        <div style={{ height: "200px" }}>
          <Chart options={options} series={[{ name: "Current L1", data: trimData(dataL1) }, { name: "Current L2", data: trimData(dataL2) }, { name: "Current L3", data: trimData(dataL3) }]} type="line" height="100%" />
        </div>
      </div>
    );
  };

  const renderGraphAccordion = () => (
    <div ref={graphRef} className="rounded-xl overflow-hidden border border-blue-400/40" style={{ boxShadow: "0 0 28px rgba(96,165,250,0.12)" }}>
      <div className="flex items-center gap-3 px-5 py-3 border-b border-slate-700/60 bg-[#0d1b2e]">
        <span className="text-[10px] font-mono font-bold text-blue-400 tracking-widest uppercase border-l-2 border-blue-400 pl-2">Node Detail</span>
        <span className="font-mono text-xs text-slate-200 font-bold uppercase truncate">
          {selectedDevice?.subgroup_mv}
          {selectedDevice?.is_incomer && <span className="ml-2 text-[9px] text-blue-400 font-normal normal-case">[Incomer]</span>}
        </span>
        <button onClick={() => setSelectedDevice(null)} className="ml-auto text-[10px] font-mono text-slate-500 hover:text-red-400 border border-slate-700 hover:border-red-400/50 px-2 py-0.5 rounded transition">✕ close</button>
      </div>
      
      <div className="grid grid-cols-2 gap-3 p-4 bg-[#161b22]">
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#0d1117] text-slate-200 overflow-hidden">
      <header className="bg-[#0d1117] border-b border-slate-700 px-5 py-4 flex justify-between items-center z-10 shrink-0">
        ...
        <h1 className="font-mono text-2xl font-bold tracking-widest text-blue-400 italic">DASHBOARD MV-A</h1>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ease-in-out ${selectedDevice ? "overflow-y-auto" : "overflow-hidden"}`}>
          <div className="flex flex-col gap-2 p-4 flex-1">
            {/* REVISI: Mapping sekarang menggunakan cardRows yang dibuat dari meterData yang dinamis */}
            {cardRows.map((rowDevices, rowIndex) => (
              <React.Fragment key={rowIndex}>
                <div className="grid gap-2 relative" style={{ gridTemplateColumns: `repeat(${CARD_COLS}, 1fr)`, flex: "1 1 0", minHeight: "130px", overflow: "visible" }}>
                  {rowDevices.map((device) => {
                    const isSelected = selectedDevice?.subgroup_mv === device.subgroup_mv;
                    const liveCard   = wsConnected ? (wsLiveData[device.subgroup_mv] ?? null) : null;
                    
                    const deviceLogs = allData[device.subgroup_mv] || [];
                    const latestLog  = deviceLogs[deviceLogs.length - 1] ?? null;
                    const powerKw    = liveCard?.power_kw   ?? latestLog?.power_kw        ?? null;
                    const energyKwh  = liveCard?.energy_kwh ?? latestLog?.consumption_kwh ?? null;

                    const isDataZeroOrError = (powerKw === 0 || powerKw === null) && (energyKwh === 0 || energyKwh === null);

                    let isOnline = null;
                    // REVISI: Pengecekan slaveId dinamis dari object `device`
                    if (device.slaveId !== 0) {
                      if (wsConnected) {
                        isOnline = !!liveCard && !isDataZeroOrError; 
                      } else {
                        const dbStatus = meterStatus[device.subgroup_mv] ?? null;
                        isOnline = (dbStatus === true) && !isDataZeroOrError;
                      }
                    }

                    let bg = "bg-[#1e293b]", border = "border-slate-700", shadow = "", dot = "bg-slate-500", nameColor = "text-slate-400";
                    if (isOnline === true)  { bg = "bg-[#0a1f14]"; border = "border-[#22c55e]"; shadow = "shadow-[0_0_6px_rgba(34,197,94,0.35)]";  dot = "bg-[#22c55e]"; nameColor = "text-[#86efac]"; }
                    if (isOnline === false) { bg = "bg-[#1f0a0a]"; border = "border-[#ef4444]"; shadow = "shadow-[0_0_6px_rgba(239,68,68,0.35)]";  dot = "bg-[#ef4444]"; nameColor = "text-[#fca5a5]"; }
                    if (isSelected)       { border = "border-blue-400"; shadow = "shadow-[0_0_14px_rgba(96,165,250,0.55)]"; bg = isOnline === true ? "bg-[#0a1f14]" : isOnline === false ? "bg-[#1f0a0a]" : "bg-[#0d1b2e]"; }

                    const anomaly     = liveCard?.anomaly      ?? false;
                    const anomalyType = liveCard?.anomaly_type ?? null;

                    if (anomaly) {
                      if (anomalyType === "POWER_LOSS")  { border = "border-orange-500"; shadow = "shadow-[0_0_18px_rgba(249,115,22,0.80)]"; }
                      else if (anomalyType === "POWER_SPIKE") { border = "border-yellow-400"; shadow = "shadow-[0_0_18px_rgba(250,204,21,0.80)]"; }
                      else { border = "border-amber-400"; shadow = "shadow-[0_0_18px_rgba(251,191,36,0.80)]"; }
                    }
                    ...
                    return (
                      <div
                        ...
                        className={`relative flex flex-col justify-between h-full p-3 border rounded-lg cursor-pointer font-mono transition-all duration-150 hover:brightness-125 hover:scale-[1.015] active:scale-[0.99] ${bg} ${border} ${shadow}`}
                      >
                        {isOnline !== null && (
                          <div className={`absolute top-2.5 right-2.5 w-2 h-2 rounded-full ${dot} ${isOnline === true ? "animate-pulse" : ""}`} />
                        )}
                        <div className="pr-4">
                          <p className={`text-[10px] font-bold uppercase leading-snug line-clamp-2 ${nameColor}`} title={device.subgroup_mv}>{device.subgroup_mv}</p>
                          {device.is_incomer && <span className="inline-block mt-1 text-[8px] font-bold uppercase text-blue-400 bg-blue-400/10 border border-blue-400/30 rounded px-1 py-0.5 leading-none">INCOMER</span>}
                        </div>
                        <div className="mt-2 space-y-1">
                          <div className="flex items-center gap-1.5"><span className="text-[9px] font-mono text-slate-600 uppercase tracking-wider w-[50px] shrink-0">POWER</span><span className="text-[11px] font-bold font-mono" style={{ color: "#22c55e" }}>{powerKw !== null ? `${powerKw.toFixed(2)} kW` : "--"}</span></div>
                          <div className="flex items-center gap-1.5"><span className="text-[9px] font-mono text-slate-600 uppercase tracking-wider w-[50px] shrink-0">ENERGY</span><span className="text-[11px] font-bold font-mono" style={{ color: "#eab308" }}>{energyKwh !== null ? `${energyKwh.toFixed(2)} kWh` : "--"}</span></div>
                        </div>
                        {anomaly && (
                          <div className={`absolute bottom-1.5 left-2 z-10 text-[7px] font-mono font-bold uppercase px-1.5 py-0.5 rounded leading-none tracking-wider ${anomalyType === "POWER_LOSS" ? "text-orange-400 bg-orange-500/10 border border-orange-500/40" : anomalyType === "POWER_SPIKE" ? "text-yellow-300 bg-yellow-400/10 border border-yellow-400/40" : "text-amber-400 bg-amber-400/10 border border-amber-400/40"}`}>⚠ {anomalyType?.replace(/_/g, " ") ?? "ANOMALY"}</div>
                        )}
                        {isSelected && <div style={{ position: "absolute", bottom: "-11px", left: "50%", transform: "translateX(-50%)", width: 0, height: 0, borderLeft: "11px solid transparent", borderRight: "11px solid transparent", borderTop: "11px solid #60a5fa", zIndex: 20 }} />}
                      </div>
                    );
                  })}
                </div>
                {selectedRowIdx === rowIndex && renderGraphAccordion()}
              </React.Fragment>
            ))}
          </div>
        </div>
          ...
          <div className="w-full min-w-[240px] flex flex-col flex-1">
            <div>
              ...
              <section className="mb-8">
                ...
                <button onClick={downloadExcel} disabled={isLoading || !allData || Object.keys(allData).length === 0} className="w-full flex items-center justify-center gap-2 text-[10px] font-mono font-bold py-2 rounded border border-green-700 text-green-400 hover:border-green-400 hover:bg-green-400/10 transition disabled:opacity-40 disabled:cursor-not-allowed"><Download size={12} />DOWNLOAD EXCEL</button>
              </section>
            </div>
