...
const TYRE_BUILDING_KEYS = ["(103- Busduct BTU 2 Tyre Building Machine BTU 2)","(103- Busduct BTU 3 Tyre Building Machine BTU 3)","(103- Busduct STU 4 Tyre Building Machine STU 4)","(103- Busduct STU 5 Tyre Building Machine STU 5)","(103- Busduct STU 6 Tyre Building Machine STU 6)","(103 C1 LV01 1)","(103 C1 LV01 2)","(MCC/ TR/ 1)"];
...
const TyreBuildingDetailChart = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Use the new service function to fetch data
    getChartData({ area: 'tyre-building' })
      .then(data => {
        setData(data);
      })
      .catch(error => {
  ...
  return (
    <motion.div className="chart-container p-4 bg-white rounded-lg shadow-xl" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      <h2 className="text-lg font-bold mb-2">Monthly Consumption Details: Tyre Building Area</h2>
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={data} margin={{ top: 20, right: 20, left: 30, bottom: 5 }} barSize={8} barGap={0}>
          ...
          {TYRE_BUILDING_KEYS.map((key, index) => (
...
export default TyreBuildingDetailChart;
