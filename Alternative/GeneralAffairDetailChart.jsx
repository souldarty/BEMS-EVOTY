...
const GA_KEYS = ["(MCC 2 Main Office AC Main aoffice)","(DB MCC #3 AC Production Office)","(MCC # 6 AC Canteen)","(907-C2-LV01 Entrance A)","(MCC # 5 AC Infirmary)","(Lighting External 2 Street Lighting Zone 1)","(DB Mech # 3 non industri - Musholla)","(DB- 901-C2-LV01 Lighting & power distribution)","(DB - GF 902-C2-LV01 Lighting & Receptacle Factory Office)","(903.C1.LV01.1)","(904 C1 LV01 1)","(906-C1-LV01 Locker Room)","(GUEST HOUSE)","(DB Mech # 2 MCC 8,MCC7,MCC 4)"];
...
const GeneralAffairDetailChart = () => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Use the new service function to fetch data
    getChartData({ area: 'general-affair' })
      .then(data => {
        setData(data);
      })
      .catch(error => {
  ...
  return (
    <motion.div className="chart-container p-4 bg-white rounded-lg shadow-xl" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
      <h2 className="text-lg font-bold mb-2">Monthly Consumption Details: G&A Area</h2>
...
export default GeneralAffairDetailChart;
